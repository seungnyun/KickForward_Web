...
<script type="text/javascript">
	function fnCmdView(recordNo) {
		document.boardForm.bno.value = recordNo;
		document.boardForm.action = 'Breakdownview';
		document.boardForm.submit("/Breakdownlist");
	}

	function fnRepair(recordNo, status) {

		$.ajax({
			type : "GET",
			url : "/updateRepairStatus/" + recordNo + "/" + status + "/",
			contentType : "application/json",
			success : function(data) {
				// 성공적으로 업데이트되면 해당 열의 수리현황을 변경
				$('#repair-status-' + recordNo).text(status);
			},
			error : function(xhr, status, error) {
				alert("수리현황 변경에 실패했습니다.");
			}
		});
	}
...
<body id="page-top">
	...
	<div class="container">
		<br>
		<h1>킥보드 고장신고</h1>
		<form:form id="boardForm" name="boardForm" method="post">
			...
			<table class="table table-sm">
				...
				<tbody>
					<c:forEach var="result" items="${list}" varStatus="status">
						<tr>
							<td>${result.bno}</td>
							<td><a href="#" onclick="fnCmdView('${result.bno}')"
								style="text-decoration-line: none">${result.title}</a></td>
							<td>${result.writerId}</td>
							<td>${result.reg_Date}</td>
							<td id="repair-status-${result.bno}">${result.repair}</td>
							<td>
								<button type="button"
									onclick="fnRepair('${result.bno}','수리준비중')" id="completeButton">수리준비중</button>&nbsp;
								<button type="button" onclick="fnRepair('${result.bno}','수리중')">수리중</button>&nbsp;
								<button type="button" onclick="fnRepair('${result.bno}','수리완료')">수리완료</button>
							</td>
						</tr>
					</c:forEach>
				</tbody>
				...
				<ul id="pageInfo" class="pageInfo">
					...
					<c:if test="${pageMaker.prev}">
						<li class="pageInfo_btn previous"><a
							href="/admin/Breakdownlist?pageNum=${pageMaker.startPage - 1}&amount=${pageMaker.cri.amount}">Previous</a>
						</li>
					</c:if>
					...
					<c:forEach var="num" begin="${pageMaker.startPage}"
						...
						<li class="pageInfo_btn"><a
							href="/admin/Breakdownlist?pageNum=${num}&amount=${pageMaker.cri.amount}"
							style="text-decoration-line: none">${num}</a></li>
					</c:forEach>

					<!--다음 페이지 이동 버튼 -->
					<c:if test="${pageMaker.next}">
						<li class="pageInfo_btn next"><a
							href="/admin/Breakdownlist?pageNum=${pageMaker.endPage + 1}&amount=${pageMaker.cri.amount}">Next</a>
						</li>
					</c:if>
				</ul>
			</div>
