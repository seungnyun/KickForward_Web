...
<body id="page-top">
		...
		<table class="table table-sm">
			...
			<tbody>
				<c:forEach var="result" items="${list}" varStatus="status">
					<tr class="color_ov">
						<td>${result.bno}</td>
						<td class="left"><a href="#" onclick="fnCmdView('${result.bno}')" style = "text-decoration-line: none">${result.title}</a></td>
				...
				<ul id="pageInfo" class="pageInfo">
					...
					<c:if test="${pageMaker.prev}">
						<li class="pageInfo_btn previous"><a
							href="/Breakdownlist?pageNum=${pageMaker.startPage - 1}&amount=${pageMaker.cri.amount}">Previous</a>
						</li>
					</c:if>
					...
					<c:forEach var="num" begin="${pageMaker.startPage}"
						...
						<li class="pageInfo_btn"><a
							href="/Breakdownlist?pageNum=${num}&amount=${pageMaker.cri.amount}"  style = "text-decoration-line: none">${num}</a></li>
					</c:forEach>

					<!--다음 페이지 이동 버튼 -->
					<c:if test="${pageMaker.next}">
						<li class="pageInfo_btn next"><a
							href="/Breakdownlist?pageNum=${pageMaker.endPage + 1}&amount=${pageMaker.cri.amount}">Next</a>
						</li>
					</c:if>
				</ul>
			</div>
	...
	<form id="moveForm" method="get">
		...
		<%-- 로그인 상태에 따라 다른 버튼 표시(로그인 한 상태이면 글작성 버튼이 보임 --%>
			<c:choose>
				<c:when test="${empty loginUser}">
				...
				<c:otherwise>
					<!-- 로그인을 한 회원만 글쓰기 버튼을 쓸수있게할것임 -->					
					<a href="Breakdownadd" class="write-link">
						<button type="button" class="addButton"><a href="Breakdownadd" style="text-decoration: none; color: white;">등록</a></button>
