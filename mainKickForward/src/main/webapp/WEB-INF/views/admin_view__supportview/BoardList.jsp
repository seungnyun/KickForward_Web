...
<script type="text/javascript">
        function fnCmdView(recordNo) {
            ...
            document.boardForm.action = 'view';
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(".move").on("click", function(e) {
			...
			moveForm.find("input[name='bno']").val(bnoValue);
        ...
        <table class="table table-sm">
            ...
            <tbody>
                <c:forEach var="result" items="${list}" varStatus="status">
                    <tr>
                        <td>${result.bno}</td>
                        <td><a href="#" onclick="fnCmdView('${result.bno}')" style = "text-decoration-line: none">${result.title}</a></td>
			...
			<ul id="pageInfo" class="pageInfo">
				...
				<c:if test="${pageMaker.prev}">
					<li class="pageInfo_btn previous"><a
						href="/admin/list?pageNum=${pageMaker.startPage - 1}&amount=${pageMaker.cri.amount}">Previous</a>
					</li>
				</c:if>
				...
				<c:forEach var="num" begin="${pageMaker.startPage}"
					...
					<li class="pageInfo_btn"><a
						href="/admin/list?pageNum=${num}&amount=${pageMaker.cri.amount}" style = "text-decoration-line: none">${num}</a></li>
				</c:forEach>

				<!--다음 페이지 이동 버튼 -->
				<c:if test="${pageMaker.next}">
					<li class="pageInfo_btn next"><a
						href="/admin/list?pageNum=${pageMaker.endPage + 1}&amount=${pageMaker.cri.amount}">Next</a>
					</li>
				</c:if>
			</ul>
		</div>
	  ...
	  <%-- 로그인 상태에 따라 다른 버튼 표시(로그인 한 상태이면 글작성 버튼이 보임 --%>
			<c:choose>
				...
				<c:otherwise>
					...
					<button class="addButton"><a href="add" style="text-decoration: none; color: white;">등록</a><br></button>
